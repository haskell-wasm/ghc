...
linkBCO interp pkgs_loaded le lb bco_ix
  ...
  foldM_ (\(!i) lit -> (unsafeWrite litsMut i =<< lookupLiteral interp pkgs_loaded le lb lit) $> succ i) 0 lits0
  lits <- unsafeFreezeIOUArray litsMut
  ptrsMut <- newSmallArrayIO (fromIntegral (sizeFlatBag ptrs0)) undefined
  foldM_ (\(!i) ptr -> (writeSmallArrayIO ptrsMut i =<< resolvePtr interp pkgs_loaded le lb bco_ix ptr) $> succ i) 0 ptrs0
